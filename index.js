        ...
        if(queryParams) {
          if(queryParams.length===0){
            ...
            for(var i=0;i<queryParams.length;i++){
              if(query[queryParams[k]]){
                queryParamsFinal+="&"+queryParams[i]+"="+query[queryParams[k]];
              }
            }
            if(req.url.indexOf("?")){
              urlToUse=req.originalUrl.split("?")[0]+queryParamsFinal;
              console.log("rul to use1"+urlToUse);
            } else {
              urlToUse=req.originalUrl.split("?")[0]+queryParamsFinal.substr(1);
              console.log("rul to use2"+urlToUse);
            }
          }
        }
        var key = urlToUse,
